    ...
    <div class="display_produce">
      ...
      <div v-if="detailInfo === null">
        ...
        <div style="transform: translate(0px, -10px)">
          <van-empty image-size="26rem" description="😭任务不见啦，快去让你的小可爱再创建一个吧！" />
        </div>
      </div>
      <div v-else>
        <van-form class="form-container">
          <div style="display: flex; justify-content: center; align-items: center">
            <van-image width="50vw" height="50vw" position="center" fit="cover" round :src="imageUrl" @click="picture" />
          </div>
          <br />
          <div style="transform: translate(0px, 8px)">
            <meta name="viewport"
              content="width=device-width, initial-scale=1.0, minimum-scale=1.0, maximum-scale=1.0,user-scalable=no" />
            <van-field label="📌任务名称" v-model="detailInfo.name" :readonly="true" />
            <van-field label="📝任务详情" v-model="detailInfo.desc" type="textarea" rows="3" :readonly="true" />
            <van-field label="💰积分价格" v-model="detailInfo.credit" :readonly="true" />
            <van-field label="🦄创建对象" v-model="detailInfo.owner[0].name" :readonly="true" />
            <van-field label="🤖任务对象" v-model="detailInfo.target[0].name" :readonly="true" />
            <van-field label="🎁完成情况" v-model="detailInfo.isDone" :readonly="true" />
            <van-field label="⏱更新时间" v-model="detailInfo.update_time" :readonly="true" />
            <van-field label="📅注册时间" v-model="detailInfo.create_time" :readonly="true" />
          </div>
        </van-form>
      </div>
    </div>
    ...
    const fetchDataAndFillForm = async () => {
      try {
        const response = await proxy.$http.get(
          `/api/task/detail?_id=${'65368cf27397894548c2cd1f'}`,
          { headers }
        );
        detailInfo.value = response.data.task
        // 填充表单字段
        imageUrl.value = data.userImage;
        if (detailInfo.value.desc == "") {
          detailInfo.value.desc = "😂这个人很懒，没有填写相关的任务详情！";
        }
