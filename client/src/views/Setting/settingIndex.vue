<template>
  <div class="content_remind">
    ...
    <div style="--van-nav-bar-icon-color: #f19290">
      <van-nav-bar title="设置中心" left-text="" left-arrow @click-left="onClickLeft" />
    </div>
    <div style="
      ...
      <div style="margin-top: 2vh; margin-bottom: 2vh">
        ...
        <van-cell-group inset>
          <van-cell center title="每日总结报告">
            <template #right-icon>
              <van-switch active-color="#f19290" v-model="checked" @change="change" />
            </template>
          </van-cell>
          <div v-if="checked == true">
            <van-cell title="提醒时间" :value="time" @click="changeTime" />
            <van-cell title="提醒结尾文案" :label="text" is-link @click="requireItem()" />
          </div>
    ...
    <div style="
      ...
      <div style="margin-top: 2vh; margin-bottom: 2vh">
        ...
        <van-cell-group inset>
          <van-cell title="海报展示设置" replace :to="`/displaySetting`" is-link />

          <van-cell title="用户信息设置" replace :to="`/UserRequire/${userId}`" is-link />

          <van-cell title="主页通知设置" replace to="/homeRequire" is-link />
          <van-cell title="用户注册设置" :label="userLogin" is-link @click="requireUser()" />
        </van-cell-group>
      </div>
    </div>
    <van-popup v-model:show="show" round position="bottom">
      <van-time-picker v-model="currentTime" title="选择时间" @confirm="confirm(currentTime)" @cancel="cancel"
        cancel-button-text=" " confirm-button-text="完成" />
    </van-popup>
  </div>
  ...
  <van-dialog v-model:show="show_3" title="修改提醒结尾" @confirm="reCheckConfirm" @cancel="reCheckCancel" show-cancel-button>
    <div style="margin-top: 3vh; margin-bottom: 4vh">
      <van-cell-group inset>
        <van-cell-group inset>
          <van-field v-model="reValue" rows="3" autosize label="提醒结尾文案" type="textarea" maxlength="100"
            placeholder="请输入提醒结尾文案" show-word-limit />
        </van-cell-group>
      </van-cell-group>
    </div>
  </van-dialog>
  <van-dialog v-model:show="show_4" title="修改用户注册密钥" @confirm="userCheckConfirm" @cancel="userCheckCancel"
    show-cancel-button>
    <div style="margin-top: 3vh; margin-bottom: 4vh">
      <van-cell-group inset>
        <van-cell-group inset>
          <van-field v-model="userValue" rows="2" autosize label="注册密钥" type="textarea" maxlength="20"
            placeholder="请输入用户注册密钥" show-word-limit />
        </van-cell-group>
      </van-cell-group>
    </div>
  </van-dialog>
